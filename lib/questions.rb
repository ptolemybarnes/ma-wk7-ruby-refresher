...
def every_possible_pairing_of_students(array)
  array.permutation(2).map(&:sort).uniq
end
...
def get_first_half_of_string(string)
  string.length.even? ? string[0...string.length/2] : string[0..string.length/2]
end

# turn a positive integer into a negative integer. A negative integer
# stays negative
def make_numbers_negative(number)
  number.abs - (number.abs * 2)
end
...
def remove_capital_letters_from_string(string)
  string.chars.select {|char| char != char.upcase || char == " " }.join
end
...
def titleize_a_string(string)
  particles = ["the", "and"]
  title = string.split(" ").map {|word| particles.include?(word) ? word : word.capitalize  }.join(" ")
...
def check_a_string_for_special_characters(string)
  string.chars.any? {|char| char.match(/\W/)}
end
